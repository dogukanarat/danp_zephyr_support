...
This is **danp_zephyr_support** - a C library template/scaffold project designed to be cloned and migrated to create new C libraries. The workflow is:
...
./test_danp_zephyr_support       # Run test executable directly
```
...
- **include/danp_zephyr_support/** - Public API headers (what users #include)
  - `danp_zephyr_support.h` - Main API header
  - `danp_zephyr_support_types.h` - Common types/definitions

- **src/** - Implementation files (not exposed to users)
  - `danp_zephyr_support.c` - Core implementation
  - `danp_zephyr_support_int.c` - Internal utilities (for helpers not in public API)
...
- **cmake/** - CMake modules
  - `danp_zephyr_supportConfig.cmake.in` - Package config template (enables find_package())
...
- CMake package config generation (enables `find_package(danp_zephyr_support)`)
...
- Headers → `/usr/local/include/danp_zephyr_support/`
- CMake config → `/usr/local/lib/cmake/danp_zephyr_support/`
...
**Test properties:**
- Test name: `danp_zephyr_support_unit_tests` (CTest)
- Timeout: 30 seconds per test
- Labels: "unit", "danp_zephyr_support"
- Test files follow structure: setUp() → tests → tearDown() → main()

**Adding new test files:**
1. Create `test/test_mymodule.c`
2. Update `test/CMakeLists.txt`:
   - Add to `target_sources(test_danp_zephyr_support PRIVATE test_mymodule.c)`
